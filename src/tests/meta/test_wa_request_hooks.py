...
and templates); the rest of the original ``test_wa_request_hooks.py`` lives in
``tests/unit/test_wa_request_hooks.py``."""
...
# Esta guarda lee el código fuente de WebAdmin, así que necesita la clase importada — y eso
# arrastra Flask. Sin el guard, una instalación sin panel web no se saltaba estos dos tests:
# se caía al colectar y no ejecutaba NADA de la suite.
try:
