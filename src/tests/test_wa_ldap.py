...
class TestLdapMapRole:
    def test_admin_group_maps_to_admin(self):
        from lib.web_admin.auth.ldap_auth import _map_role
...
class TestLdapAuthenticate:
    ...
    def test_connection_error_returns_connection_error(self, admin, config_dir):
        ...
        with patch('lib.web_admin.auth.ldap_auth._HAS_LDAP3', True), \
             patch('lib.web_admin.auth.ldap_auth.Server') as MockServer, \
             patch('lib.web_admin.auth.ldap_auth.Connection') as MockConn:
    ...
    def test_user_not_found_returns_not_found(self, admin, config_dir):
        ...
        _ldap_cfg(config_dir)
        with patch('lib.web_admin.auth.ldap_auth._HAS_LDAP3', True), \
             patch('lib.web_admin.auth.ldap_auth.Server'), \
             patch('lib.web_admin.auth.ldap_auth.Connection') as MockConn:
            conn_inst = MagicMock()
            conn_inst.entries = []
