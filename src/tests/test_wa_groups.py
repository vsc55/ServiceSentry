...
class TestGroups:
    ...
    def test_update_builtin_group_label_ignored(self, client):
        ...
        _login(client)
        resp = client.put("/api/v1/groups/administrators", json={"label": "Hacked"})
        assert resp.status_code == 200
    ...
    def test_update_builtin_group_roles(self, client):
        ...
        _login(client)
        resp = client.put("/api/v1/groups/administrators", json={"roles": ["admin", "editor"]})
        assert resp.status_code == 200
        groups = client.get("/api/v1/groups").get_json()
    ...
    def test_cannot_delete_builtin_group(self, client):
        _login(client)
        resp = client.delete("/api/v1/groups/administrators")
    ...
    def test_non_builtin_groups_have_builtin_false(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "custom_g", "label": "Custom", "roles": []})
        groups = client.get("/api/v1/groups").get_json()
    ...
    def test_create_group_duplicate_name(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "dup", "label": "D", "roles": []})
        resp = client.post("/api/v1/groups", json={"name": "dup", "label": "D2", "roles": []})
        assert resp.status_code == 409

    def test_create_group_name_normalised(self, admin, client):
        """Name is lowercased and spaces become underscores."""
        _login(client)
        client.post("/api/v1/groups", json={"name": "My Group", "label": "My Group", "roles": []})
        assert "my_group" in admin._groups

    def test_update_group_label(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "grp1", "label": "Old", "roles": []})
        resp = client.put("/api/v1/groups/grp1", json={"label": "New Label"})
        assert resp.status_code == 200
        groups = client.get("/api/v1/groups").get_json()
    ...
    def test_update_group_roles(self, client):
        ...
        client.put("/api/v1/groups/flexgrp", json={"roles": ["viewer", "editor"]})
        groups = client.get("/api/v1/groups").get_json()
        assert set(groups["flexgrp"]["roles"]) == {"viewer", "editor"}

    def test_update_group_description(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "descgrp", "label": "D", "roles": []})
        client.put("/api/v1/groups/descgrp", json={"description": "New desc"})
        groups = client.get("/api/v1/groups").get_json()
        assert groups["descgrp"]["description"] == "New desc"

    def test_update_nonexistent_group(self, client):
        _login(client)
        resp = client.put("/api/v1/groups/ghost", json={"label": "x"})
        assert resp.status_code == 404

    def test_delete_group(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "todelte", "label": "D", "roles": []})
        resp = client.delete("/api/v1/groups/todelte")
        assert resp.status_code == 200
        assert resp.get_json()["ok"] is True
        groups = client.get("/api/v1/groups").get_json()
    ...
    def test_create_group_requires_groups_add_perm(self, client):
        ...
        resp = client.post("/api/v1/groups", json={"name": "forbidden", "label": "F", "roles": []})
        assert resp.status_code == 403

    def test_delete_group_requires_groups_delete_perm(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "protected", "label": "P", "roles": []})
        client.post("/api/v1/roles", json={
        ...
        resp = client.delete("/api/v1/groups/protected")
        assert resp.status_code == 403

    def test_groups_persisted_to_disk(self, admin, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "persist_grp", "label": "P", "roles": ["viewer"]})
    ...
    def test_api_me_includes_groups(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "my_grp", "label": "My", "roles": []})
        client.put("/api/v1/users/admin", json={"groups": ["my_grp"]})
        me = client.get("/api/v1/me").get_json()
        assert "my_grp" in me["groups"]

    def test_api_get_users_includes_groups(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "usr_grp", "label": "U", "roles": []})
        client.post("/api/v1/users", json={
    ...
    def test_update_user_groups(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "upd_grp", "label": "U", "roles": []})
        client.post("/api/v1/users", json={"username": "upd_user", "password": "testpass", "role": "viewer"})
        client.put("/api/v1/users/upd_user", json={"groups": ["upd_grp"]})
        users = client.get("/api/v1/users").get_json()
    ...
    def test_group_audit_events(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "audit_grp", "label": "A", "roles": []})
        client.delete("/api/v1/groups/audit_grp")
...
class TestGroupPermissions:
    ...
    def test_multiple_groups_merged(self, admin, client):
        ...
        _login(client)
        client.post("/api/v1/groups", json={"name": "grp_a", "label": "A", "roles": ["editor"]})
        client.post("/api/v1/groups", json={"name": "grp_b", "label": "B", "roles": ["viewer"]})
    ...
    def test_groups_edit_perm_allows_update_group(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "edit_target", "label": "E", "roles": []})
        client.post("/api/v1/roles", json={
        ...
        resp = client.put("/api/v1/groups/edit_target", json={"label": "Updated"})
        assert resp.status_code == 200
...
class TestGroupInputValidation:
    ...
    def test_update_group_unknown_role_rejected(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "upd_role_grp", "label": "U", "roles": []})
        resp = client.put("/api/v1/groups/upd_role_grp", json={"roles": ["FAKE_ROLE"]})
        assert resp.status_code == 400
        assert "error" in resp.get_json()

    def test_update_group_unknown_role_not_persisted(self, admin, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "persist_role_grp", "label": "P", "roles": ["viewer"]})
        client.put("/api/v1/groups/persist_role_grp", json={"roles": ["viewer", "FAKE"]})
        viewer_uid = admin._role_name_to_uid("viewer")
        assert admin._groups["persist_role_grp"]["roles"] == [viewer_uid]

    def test_update_group_non_list_roles_rejected(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "nl_role_grp", "label": "NL", "roles": []})
        resp = client.put("/api/v1/groups/nl_role_grp", json={"roles": "viewer"})
        assert resp.status_code == 400

    def test_update_group_valid_roles_accepted(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "vr_grp", "label": "VR", "roles": ["viewer"]})
        resp = client.put("/api/v1/groups/vr_grp", json={"roles": ["viewer", "editor"]})
        assert resp.status_code == 200
        groups = client.get("/api/v1/groups").get_json()
    ...
    def test_update_group_unknown_member_rejected(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "mem_grp", "label": "M", "roles": []})
        resp = client.put("/api/v1/groups/mem_grp", json={"members": ["ghost_user"]})
        assert resp.status_code == 400
        assert "error" in resp.get_json()

    def test_update_group_unknown_member_not_persisted(self, admin, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "safe_mem_grp", "label": "S", "roles": []})
        client.put("/api/v1/groups/safe_mem_grp", json={"members": ["nobody"]})
        members = [
    ...
    def test_update_group_non_list_members_rejected(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "nl_mem_grp", "label": "NL", "roles": []})
        resp = client.put("/api/v1/groups/nl_mem_grp", json={"members": "admin"})
        assert resp.status_code == 400

    def test_update_group_valid_member_accepted(self, admin, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "real_mem_grp", "label": "R", "roles": []})
        resp = client.put("/api/v1/groups/real_mem_grp", json={"members": ["admin"]})
        assert resp.status_code == 200
    ...
    def test_update_group_mixed_valid_and_invalid_members_rejected(self, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "mix_mem_grp", "label": "MM", "roles": []})
        resp = client.put("/api/v1/groups/mix_mem_grp", json={"members": ["admin", "ghost_user"]})
        assert resp.status_code == 400

    def test_update_group_empty_members_accepted(self, admin, client):
        _login(client)
        client.post("/api/v1/groups", json={"name": "empty_mem_grp", "label": "E", "roles": []})
        client.put("/api/v1/groups/empty_mem_grp", json={"members": ["admin"]})
        resp = client.put("/api/v1/groups/empty_mem_grp", json={"members": []})
        assert resp.status_code == 200
