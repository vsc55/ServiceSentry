...
class TestTheImportCycleStaysOpen:
    ...
    def _flask_probe(module):
        ...
        # A fresh subprocess is the point — a real second interpreter is the only way to see
        # what an import pulls in without this process's already-loaded modules masking it.
        # But spawning a heavy import ALONGSIDE a saturated `-n auto` run (every core taken by
        # xdist workers) is fragile on some machines: the extra process can be starved so long
        # that even a 600s cap expires — the ~3s isolated import never finishes because it is
        # barely scheduled. That is contention, not a hang, and it must not fail the build.
        #
        # So the timeout is read differently by context. Under xdist it means "the machine was
        # too busy to run the probe" → skip; the property is engine- and parallelism-independent
        # and still checked on every `-n0` run and on CI. Under `-n0` there is nothing to
        # contend with, so a timeout there is a genuine hang and stays a failure.
        try:
            r = subprocess.run([sys.executable, '-c', code], cwd=SRC,
                               capture_output=True, timeout=120)
        except subprocess.TimeoutExpired:
            if int(os.environ.get('PYTEST_XDIST_WORKER_COUNT', '1')) > 1:
                pytest.skip('import probe starved under xdist contention — run -n0 to check it')
            raise
    ...
    def test_the_catalog_is_still_imported_the_same_way(self):
        ...
        assert len(PERMISSIONS) == 92 and len(PERMISSION_GROUPS) == 24
