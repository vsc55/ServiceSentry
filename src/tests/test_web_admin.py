...
class TestPermissionsConstants:
    ...
    def test_permissions_tuple_has_15_flags(self):
    ...
    def test_editor_permissions(self):
        ...
        # Editor must NOT have user/role/session management
        assert 'users_view' not in ep
    ...
    def test_viewer_has_no_permissions(self):
        from lib.web_admin.app import BUILTIN_ROLE_PERMISSIONS
        assert BUILTIN_ROLE_PERMISSIONS['viewer'] == frozenset()
    ...
    def test_get_role_permissions_custom_role(self, admin):
    ...
    def test_api_me_viewer_has_no_permissions(self, admin, client):
...
class TestCustomRoles:
    ...
    def test_builtin_roles_have_permissions(self, client):
        ...
        assert set(roles['viewer']['permissions']) == set()
    ...
    def test_cannot_update_builtin_role(self, client):
        _login(client)
        resp = client.put("/api/roles/admin", json={"label": "Hacked"})
        assert resp.status_code == 400
...
class TestGranularPermissions:
    ...
    @staticmethod
    def _make_user_with_perms(admin, username, perms: list[str], password="pass"):
