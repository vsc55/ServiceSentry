...
def register(app, wa):
    ...
    def api_create_user():
        ...
        user_landing = str(data.get('landing_page', '') or '').strip()
        if user_landing and user_landing not in home_page_ids():
        ...
        if user_landing:
            wa._users[uname]['landing_page'] = user_landing
        if group_uids:
    ...
    def api_update_user(username: str):
        ...
        touch_entity(user, session.get('username', SYSTEM_USER))
        wa._persist_users()
        if changes:
