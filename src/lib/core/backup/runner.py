...
class BackupRunner:
    ...
    def tick(self, now_ts: float | None = None) -> dict:
        ...
        # only when something is due, so the 143 idle ticks a day cost no contention at all.
        if not self._claim():
            out['leader'] = False
            return out
        ...
        for task in due:
            ...
            self._run_task(task, now, var_dir, backup_dir, out)
        return out
    ...
    def _run_task(self, task: dict, now: float, var_dir: str, backup_dir: str,
                  out: dict, progress_cb=None) -> None:
        wa = self._wa
        ...
        # Pruned AFTER the new one is on disk, and only within THIS task: a daily task counting
        # every automatic copy would prune the monthly one's — deleting exactly the copies that
        # took a month to become worth having. Pruning first would, on a full disk, delete the
        # old copy and fail to write the new one.
        gone = []
        for old in _sched.prune(_svc.list_backups(var_dir, backup_dir),
                                task.get('keep'), task.get('name')):
    ...
    def _tasks(self) -> list:
        ...
        store.upsert({
            ...
            'keep': getattr(self._wa, '_BACKUP_KEEP', 7),
        }, actor='(migration)')
