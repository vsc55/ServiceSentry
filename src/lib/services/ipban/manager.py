#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Internal fail2ban wiring for WebAdmin (the host side of the ipban service).

The jail/counter *logic* lives in :mod:`lib.services.ipban.jail` (framework-free)
and its persistence in :mod:`lib.services.ipban.store` (bans + whitelist).
This mixin is only the Flask-side glue mixed into :class:`WebAdmin`:

  * create the shared :class:`~lib.services.ipban.jail.IpBanManager` + its stores,
...
It is the ipban service's ``manager.py`` (same role as :mod:`lib.services.syslog.manager`);
the Services-tab surface + heartbeat live alongside it in ``embedded.py``.
...
class _IpBanMixin:
    ...
    # HTTP block actions the web service can produce (see lib.services.ipban.exposed).
