...
class BaseConnector(ABC):
    ...
    # Dialect tag for the few places that need portable-SQL branching (last-insert-id,
    # JSON extraction). Overridden by each concrete connector.
    KIND: str = 'sqlite'

    def last_insert_id(self) -> int:
        """Id generated by the most recent INSERT on this thread's connection.

        Connection/session-scoped and portable across the three backends: safe to call
        right after ``execute(INSERT ...)`` (+ ``commit()``) on the same thread — unlike a
        racy ``SELECT MAX(id) WHERE ...`` that two concurrent producers can both read.
        """
        q = {'mysql': 'SELECT LAST_INSERT_ID()',
             'postgresql': 'SELECT lastval()'}.get(self.KIND, 'SELECT last_insert_rowid()')
        row = self.fetchone(q)
    ...
    def _rebuild_copy_plan(self, spec: TableSpec, actual_cols: list[ColumnInfo]):
        """Compute the copy plan for a rebuild: ``(tmp_name, extras, collist, select_list,
        has_common)``.  Shared by the base rebuild and the MySQL override."""
        q = self.quote_ident
        ...
        return tmp, extras, collist, ', '.join(select_exprs), bool(common)
    ...
    def _apply_rebuild(
        ...
        Used by SQLite and PostgreSQL, where DDL is transactional so this is atomic;
        MySQL overrides it (DDL auto-commits there) with an atomic ``RENAME TABLE`` swap.
        """
        q = self.quote_ident
