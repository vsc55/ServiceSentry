...
class SnmpSampler:
    ...
    def _sample_metric(self, metric: dict, conn: dict, now: float,
        ...
        if metric.get('oid'):
            raw, err = self._snmp_get(oid=metric['oid'], **conn)
            if err:
                return False, err
            self._store_value(rows, '', '', metric, raw, now, state)
            return True, None

        walked, err = self._snmp_walk_oid(oid=metric['walk'], **conn)
        ...
        for extra in idx_oids + ([by_oid] if by_oid else []):
            if extra not in columns:
                found, _e = self._snmp_walk_oid(oid=extra, **conn)
        ...
        for index, raw in walked.items():
            ...
            if name:
                ...
                row_name = row_key
            self._store_value(rows, row_key, row_name, metric, raw, now, state,
