...
class IdentityChecks:
    ...
    def _check_licenses(self, it: dict, key: str, label: str, token: str, timeout: int) -> None:
        ...
        ``licenses_free_min`` (0 = warn only when a SKU is fully exhausted)."""
        try:
        ...
        threshold = self._threshold(it, 'licenses_free_min')
        if not skus:
    ...
    def _check_secrets(self, it: dict, key: str, label: str, token: str, timeout: int) -> None:
        ...
        when the soonest-expiring credential is within ``secret_expiry_days`` days."""
        ...
        try:
            ...
            return
        days_warn = self._threshold(it, 'secret_expiry_days', 30)
    ...
    def _check_secure_score(self, it: dict, key: str, label: str, token: str, timeout: int) -> None:
        ...
        below ``secure_score_min`` (0 = informational only)."""
        try:
        ...
        smin = self._threshold(it, 'secure_score_min')
    ...
    def _check_risky_users(self, it: dict, key: str, label: str, token: str, timeout: int) -> None:
        ...
        at risk exceeds ``risky_users_max``."""
        ...
        threshold = self._threshold(it, 'risky_users_max')
