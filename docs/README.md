...
| [explica-dcim.md](explica-dcim.md) | **Plan (no construido).** Inventario físico: datacenters, salas y racks con sus U por cara, potencia, cableado con etiquetas y enlaces entre sedes, **con la pertenencia (empresa) como eje aparte de la contención** para racks compartidos entre sociedades — alimentado con lo que ya miden las sondas, y con la discrepancia entre lo declarado y lo medido como eje. Modelo, catálogo devicetype-library, permisos y fases |
...
| [ref-cli.md](ref-cli.md) | CLI de gestión one-shot: subcomandos `user`/`group` (alta/baja/rol/contraseña/grupos, reset del segundo factor), `status` y `reload` de servicios; contexto headless, capa de servicio compartida en `lib/core` (rutas web + CLI) y auto-discovery de servicios |
| [ref-modulos.md](ref-modulos.md) | Los 20 módulos integrados: referencia de configuración, campos y flujo de cada uno |
| [explica-web-admin.md](explica-web-admin.md) | Interfaz web Flask: características, roles (76 permisos), notificaciones, syslog, eventos, seguridad, endpoints REST, i18n, formularios por schema |
...
| [ref-permisos.md](ref-permisos.md) | Fuente única del RBAC: catálogo de los 76 flags de permiso, roles integrados/personalizados, grupos, permisos dinámicos (módulo/servidor/cluster) y estructuras internas |
...
| **Segundo factor / MFA** (TOTP, política, códigos de recuperación, WebAuthn) | [explica-mfa.md](explica-mfa.md) · [explica-seguridad.md](explica-seguridad.md) (dónde encaja en el login) · [ref-api.md](ref-api.md) (endpoints) · [ref-configuracion.md](ref-configuracion.md) (claves y `SS_*`) |
...
- Incluye **interfaz web de administración** (Flask) con RBAC (76 permisos), grupos, modo oscuro, historial con gráficas e i18n.
