...
| `web_admin.api_token_log_max` | int | `200` | Llamadas recientes que se guardan **por token** (0–10000), para poder ver qué ha hecho cada uno. Es un anillo: las viejas se descartan, así que la tabla no crece con el tráfico. Por token y no global, para que uno hablador no desaloje el historial de uno tranquilo. `0` lo apaga. Config › Autenticación › Tokens de API |
...
**Clave secreta compartida (`SS_SECRET_KEY`):** firma las cookies de sesión y **deriva la clave Fernet con la que se cifra cada secreto guardado**. Sin definirla se genera una sola vez en `.flask_secret` dentro de `config_dir`, lo cual basta mientras ese directorio persista y lo compartan todos los procesos —los `docker compose` montan el mismo volumen `config` en los cuatro servicios—. **Defínela cuando el fichero no se pueda compartir ni conservar**: un pod por rol en Kubernetes (sin volumen común) o cualquier despliegue donde el volumen de config se pueda recrear. Todos los procesos que hablen con la misma BD tienen que llevar **el mismo valor**; si no, lo que cifra uno no lo descifra el otro. Son 64 caracteres hex (`python -c "import secrets; print(secrets.token_hex(32))"`) y un valor mal formado **detiene el arranque** a propósito: arrancar con una clave distinta de la que fijaste es como los secretos se vuelven ilegibles sin que nada avise. Perderla es perder todos los secretos almacenados.
