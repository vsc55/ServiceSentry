...
**Archivo:** `tests/meta/test_snmp_mib_manager_screen.py` — 140 tests
...
| `TestWhatDeletingLeavesBehind::*` (8) | Borrar una carpeta de fuentes no se lleva lo compilado de ella —`compiled/` es plano y sus ficheros llevan el nombre del módulo, así que nada dice qué `.py` salió de dónde— y esos restos se contaban como **dependencias**: 374 de una clase escondidos entre 3 de la otra. Ahora son un estado propio, con su chip, su grupo (una carpeta que ya no existe no es una carpeta que listar) y su limpieza: **primero el informe y luego el borrado**, porque un número que nadie puede comprobar no es una confirmación. Las guardas cubren lo que volvería a colarse — que una **dependencia** que pysmi resolvió fuera de `raw/` no se barra con ellos, que los nombres se comparen por lo que los ficheros **declaran** y no por cómo se llaman, que la carpeta vaciada se vaya de dentro afuera, que el catálogo de símbolos se tire, y que de las dos acciones sólo una sea de sólo lectura |
...
| `TestImportingIsItsOwnView::*` (10) | Meter MIBs era un panel plegado encima de la lista, y plegar es lo que se hace cuando no hay otro sitio donde ponerlo: cerrado seguía costando una fila y no decía nada, abierto se comía media altura de lo mismo que ayudaba a llenar. Lo que zanjó la discusión es lo que contesta «Comparar»: **una fila por fichero del archivo del fabricante**, que contra LibreNMS son cuatro mil. Un panel enseñaba nueve y un diálogo treinta; eso es una **página**. Así que la sección declara **dos vistas** —y las nombra en el lang del módulo, porque el núcleo no tiene cadenas que nombren la vista de un módulo—; los controles de importación están **una sola vez** en el fichero (dos copias vivas de un `id` es un control que rellena una caja y lee la otra); el renderizador **pinta la vista que dice la URL** (una vista es una sub-ruta, y las rutas son del núcleo) y clona una vez por vista, para no perder la carpeta abierta y el buscador al volver; **las dos entradas llevan al mismo sitio** —el botón de la barra navega en vez de pintar, así que URL, resaltado y miga de pan son los mismos por cualquiera de los dos caminos, y ambos dicen la misma palabra—; el informe **llena la página** (`.ss-vfill` hasta un solo `.ss-vscroll`); y **un trabajo en marcha sobrevive al cambio de vista**: las dos vistas llevan los mismos `id` de progreso a propósito —un `<template>` es inerte y sólo vive el clonado—, así que una descarga lanzada en importación sigue contando en la lista en vez de parecer parada |
