...
| CRUD del registro de servidores | `servers_view/add/edit/delete` |
| CRUD de credenciales reutilizables | `credentials_view/add/edit/delete` |
| Fijar default global / restaurar fábrica del Overview | `overview_set_default` / `overview_reset_factory` |
| Ver / vaciar syslog (mensajes y descartes) | `syslog_view` / `syslog_delete` |
...
- Se persisten en la tabla `roles` de la base de datos.
...
se reaplica **en caliente** al guardar el valor desde el panel (`routes/config.py`),
sin reiniciar.
