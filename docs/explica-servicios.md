...
> general — ver [explica-descubrimiento.md → Servicios embebidos](explica-descubrimiento.md#3-servicios-embebidos-embedded_service).
> La *ejecución de checks* del monitor está en [explica-arquitectura.md → Ejecución de checks](explica-arquitectura.md#ejecución-de-checks-un-único-ejecutor).
...
panel (ver [explica-seguridad.md → fail2ban](explica-seguridad.md#fail2ban-interno-bans-de-ip-a-nivel-de-servicio)),
...
> [explica-notificaciones.md → notificación agrupada por ciclo](explica-notificaciones.md#el-monitor-notificación-agrupada-por-ciclo-monitornotifier)
> y [→ Severidad warning](explica-notificaciones.md#severidad-warning).
...
Permisos: ver la [pestaña Services en explica-web-admin.md](explica-web-admin.md#servicios). El estado se
...
**servidor de control** (sin Flask): `control_server.py` — un `ThreadingHTTPServer` (stdlib)
que arranca si hay `SS_CONTROL_TOKEN`, con `/control/health` (sin auth, probe k8s), `/control/info`
y `/control/reconcile` (Bearer). El **contrato HTTP** de estos endpoints está en
[ref-api.md](ref-api.md) (§ «Plano de control inter-proceso»); aquí importa el *porqué*: son
la vía de diagnóstico y el objetivo del *poke* de reconcile.
...
- [caso-docker.md](caso-docker.md) — topologías monolítica / microservicios / microservicios + Traefik.
- [caso-kubernetes.md](caso-kubernetes.md) — un Deployment por rol, probes contra `/control/health`, NetworkPolicy.
- [caso-despliegue.md](caso-despliegue.md) — gestión de servicios (systemd/OpenRC) y comandos de servicio.
