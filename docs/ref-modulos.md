...
| `states.<v>.icon` | La **marca** que se dibuja para ese estado (`bi-…`). La pantalla dibuja una por `level` y para `info` no dibuja ninguna a propósito: `info` es el nivel de «esto es lo que contestó», y **todo** tipo de interfaz es `info` —una VLAN no es mejor ni peor que un puerto—, así que un punto gris delante de los seis es seis píxeles de nada repetidos. «Apagado» también es `info` y es justo el caso contrario: ni bueno ni malo, y lo único que quien recorre con la vista un rail de interfaces quiere distinguir. Cuál es cuál solo lo sabe el perfil, así que lo dice el perfil |
